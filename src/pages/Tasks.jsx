...
import { useGetPostQuery } from '../redux/features/baseApi/api';
...
const Tasks = () => {
  ...
  const runningTasks = data?.filter((item) => item.status == 'running');
  const doneTasks = data?.filter((item) => item.status == 'done');
